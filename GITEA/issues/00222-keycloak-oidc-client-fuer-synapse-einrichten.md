...
source: slarti/backlog#222
state: closed
updated_at: 2026-07-15T02:01:26+02:00
is_epic: false
labels:
  - "FLASH_FREE"
  - "sub-task"
...
# Keycloak-OIDC Client fuer Synapse einrichten

Parent: #212. Depends on: #220.

## FLASH_FREE packet
Repo: `architecture`; branch: `docs/matrix-keycloak-oidc`; create `docs/matrix/keycloak-oidc.md` and `pi/keycloak/matrix-client-realm-export.example.json`.
Implement: declarative Keycloak client template in a non-master realm, confidential authorization-code flow, Synapse callback, stable subject mapping and minimal claims.
Tests first: JSON parse plus documentation assertions in `pi/tests/test_matrix_oidc.sh`.
Do not: call Keycloak admin API, create realms/clients, store client secret.
Done: exact operator steps and a login/logout smoke test.
...
- Repository: `architecture`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `docs/matrix-keycloak-oidc`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `docs/matrix/keycloak-oidc.md; pi/keycloak/matrix-client-realm-export.example.json; pi/tests/test_matrix_oidc.sh`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `bash pi/tests/test_matrix_oidc.sh`.
