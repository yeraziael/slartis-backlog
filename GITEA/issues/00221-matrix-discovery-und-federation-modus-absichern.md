...
source: slarti/backlog#221
state: closed
updated_at: 2026-07-15T01:13:46+02:00
is_epic: false
labels:
  - "FLASH_FREE"
  - "sub-task"
...
# Matrix Discovery und Federation-Modus absichern

Parent: #212. Depends on: #220.

## FLASH_FREE packet
Repo: `architecture`; branch: `feat/matrix-discovery-private`; create `pi/synapse/well-known/` examples and `docs/matrix/federation-mode.md`.
Implement: client/server well-known response contract and explicit private/no-federation Synapse settings. Explain the reversible activation procedure.
Tests first: `pi/tests/test_matrix_discovery.sh` validates JSON, CORS expectation and private-mode config.
Do not: publish well-known files or open federation.
Done: external test instructions demonstrate client discovery and federation denial.
...
- Repository: `architecture`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `feat/matrix-discovery-private`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `pi/synapse/well-known/; docs/matrix/federation-mode.md; pi/tests/test_matrix_discovery.sh`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `bash pi/tests/test_matrix_discovery.sh`.
