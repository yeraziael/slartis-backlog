...
# Matrix Parallelbetrieb mit Telegram und Signal testen

Parent: #212. Depends on: #232, #233, #234, #235, #237.

## FLASH_FREE packet
Repo: `home-repo`; branch: `test/matrix-parallel-runbook`; create `docs/matrix/parallel-operation-test.md` and test fixtures.
Implement: seven-day operator runbook with daily health/resource checks, identical cross-channel command matrix, outage/restart/duplicate/loop scenarios and acceptance worksheet.
Tests first: `runtime/tests/test_matrix_parallel_fixtures.py` validates every scenario has action, expected result and evidence field.
Do not: declare Matrix primary or alter external gateways.
Done: all success thresholds and explicit abort/rollback triggers are measurable.
...
- Repository: `home-repo`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `test/matrix-parallel-runbook`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `docs/matrix/parallel-operation-test.md; test fixtures; runtime/tests/test_matrix_parallel_fixtures.py`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `python3 runtime/tests/test_matrix_parallel_fixtures.py`.
