...
source: slarti/backlog#235
state: closed
updated_at: 2026-07-15T08:25:29+02:00
is_epic: false
labels:
  - "FLASH_FREE"
  - "sub-task"
...
# Matrix Backup und Restore-Test

Parent: #212. Depends on: #218.

## FLASH_FREE packet
Repo: `architecture`; branch: `feat/matrix-backup-restore`; create `pi/backupMatrix.sh`, `pi/restoreMatrix.md`, `pi/tests/test_matrix_backup.sh`.
Implement: backup manifest for DB dump, media, signing keys, config and adapter state; restrictive permissions, retention and checksum verification. Restore procedure targets isolated paths first.
Tests first: static test verifies all required artifact classes, `umask 077`, no secrets printed and restore order documented.
Do not: run backup/restore against Pi or delete old data.
Done: operator blackbox restore test has measurable success criteria.
...
- Repository: `architecture`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `feat/matrix-backup-restore`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `pi/backupMatrix.sh; pi/restoreMatrix.md; pi/tests/test_matrix_backup.sh`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `bash pi/tests/test_matrix_backup.sh`.
