...
source: slarti/backlog#226
state: closed
updated_at: 2026-07-15T11:58:04+02:00
is_epic: false
labels:
  - "FLASH_FREE"
  - "sub-task"
...
# Matrix Inbound Adapter fuer Lydia implementieren

Parent: #212. Depends on: #224, #225.

## FLASH_FREE packet
Repo: `home-repo`; branch: `feat/matrix-inbound-adapter`; create `runtime/matrix/inbound.py` and `runtime/matrix/client.py`.
Implement: long-poll/sync event filter, text/reply extraction and Envelope v1 conversion; unknown events are ignored with structured reason. Interface: `poll_once(cursor) -> list[Envelope]`.
Tests first: `runtime/tests/test_matrix_inbound.py` covers text, reply, unknown event, invalid payload and HTTP rate-limit mapping using local fixtures.
Do not: wire processor daemon or use production token.
Done: no Matrix-specific business logic enters command handlers.
...
- Repository: `home-repo`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `feat/matrix-inbound-adapter`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `runtime/matrix/inbound.py; runtime/matrix/client.py; runtime/tests/test_matrix_inbound.py`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `python3 runtime/tests/test_matrix_inbound.py`.
