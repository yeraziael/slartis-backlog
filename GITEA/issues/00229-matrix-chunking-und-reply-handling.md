...
source: slarti/backlog#229
state: closed
updated_at: 2026-07-15T13:51:50+02:00
is_epic: false
labels:
  - "FLASH_FREE"
  - "sub-task"
...
# Matrix Chunking und Reply Handling

Parent: #212. Depends on: #228.

## FLASH_FREE packet
Repo: `home-repo`; branch: `feat/matrix-chunking-replies`; create `runtime/matrix/chunking.py`; register the test through `ci-manifest.yaml` and regenerate `Makefile` plus `.gitea/workflows/ci.yaml` with `ci-generate.py`.
Implement: configurable Matrix response limit; retain word boundaries, preserve fenced-code blocks where possible, prefix only multipart output `[i/n]`.
Tests first: `runtime/tests/test_matrix_chunking.py` covers short text, exact boundary, multi-part numbering, long word and code fence. Extend `runtime/tests/test_ci_generator.py` to prove the focused test is present in the manifest-generated Makefile and workflow.
Do not: change Telegram/Signal chunkers.
Done: each chunk is independently sendable and ordering is deterministic.
...
- Repository: `home-repo`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `feat/matrix-chunking-replies`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `runtime/matrix/chunking.py; runtime/tests/test_matrix_chunking.py; ci-manifest.yaml; Makefile; .gitea/workflows/ci.yaml; runtime/tests/test_ci_generator.py`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `python3 runtime/tests/test_matrix_chunking.py && python3 runtime/tests/test_ci_generator.py`.
