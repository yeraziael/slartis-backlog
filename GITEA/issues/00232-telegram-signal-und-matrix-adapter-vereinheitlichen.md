...
# Telegram-, Signal- und Matrix-Adapter vereinheitlichen

Parent: #212. Depends on: #226, #228, #225.

## FLASH_FREE packet
Repo: `home-repo`; branch: `refactor/channel-envelope-adapters`; add `runtime/messaging/router.py`; adapt Telegram/Signal only through narrow adapters.
Implement: one parser/dispatcher consumes Envelope v1; adapters map inbound/outbound data without channel-specific commands. Preserve current external behavior.
Tests first: extend `runtime/tests/test_message_envelope.py` and existing messenger tests with identical `/status` fixture per channel.
Do not: remove legacy path before #238 or alter external API limits.
Done: all three adapters pass the same command contract.
...
- Repository: `home-repo`. Exakte Worktree(s): `<local-path-redacted>`. Branch: `refactor/channel-envelope-adapters`. Vor jedem Editieren exakt `git status --short --branch` ausfuehren.
- Unrelated working-tree changes nicht zuruecksetzen oder ueberschreiben; melden und stoppen.
- Erlaubt sind nur die oben genannten Paketpfade sowie die folgenden fokussierten Testpfade: `runtime/messaging/router.py; the narrow Telegram/Signal/Matrix adapter files required by the packet; runtime/tests/test_message_envelope.py; existing messenger tests`. Generierte Dateien duerfen nur ueber den vorhandenen Generator entstehen.
- Keine ungeplanten Refactorings, Adapter-Umbauten oder Aenderungen an anderen Issues.

### Verbindliche Nachweise
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `python3 runtime/tests/test_message_envelope.py && bash runtime/tests/test_messenger_base.sh && bash runtime/tests/test_processor_handlers.sh`.
