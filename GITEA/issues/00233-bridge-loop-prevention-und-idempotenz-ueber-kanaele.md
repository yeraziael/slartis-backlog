...
# Bridge-Loop-Prevention und Idempotenz ueber Kanaele

Parent: #212. Depends on: #232, #227.

## FLASH_FREE packet
Repo: `home-repo`; branch: `feat/bridge-loop-guard`; create `runtime/messaging/loop_guard.py`.
Implement: reject envelopes with repeated trace, self-origin, or bridge hop above configured maximum; preserve original trace across channels.
Tests first: `runtime/tests/test_loop_guard.py` covers single bridge, repeated trace, max hop, absent trace and replay.
Do not: rely on display text markers or silently drop without audit reason.
Done: decision is deterministic and auditable.
...
- Fokus-Test aus dem jeweiligen Repository-Root, exakt und in dieser Form: `python3 runtime/tests/test_loop_guard.py`.
